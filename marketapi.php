<?php
include('header.php');
if (isset($_POST['link']) && isset($_POST['market_id'])) {
	$link = $_POST['link'];
	$id = $_POST['market_id'];
...
/* 	$success = false;
	while ($success == false) {
		try {
			$data = file_get_contents("http://steamcommunity.com/market/priceoverview/?appid=730&currency=1&market_hash_name=$link");
			if ($data != false) {
				$success = true;
...
// $context = stream_context_create($opts);
	...
	//while ($success == false) {
		try {
			$lowest = $actions->getLowestPrice($link);
			echo isset($lowest['lowest_price']) ? $lowest['lowest_price'] : " ";
			// if (isset($lowest['lowest_price'])) {
				// echo $lowest['lowest_price'];
				// $success = true;
				// $json = json_decode($data);
				// $array['market_id'] = $id;
				// $array['link'] = $link;
